...
def edit_smpp_connector(cid):
    ...
    if form.accepted:
        ret = jasmin.connector(
            [
                "update",
                con,
                form.vars["c_ripf"],
                form.vars["c_con_fail_delay"],
                form.vars["c_dlr_expiry"],
                form.vars["c_coding"],
                form.vars["c_logrotate"],
                form.vars["c_submit_throughput"],
                form.vars["c_elink_interval"],
                form.vars["c_bind_to"],
                form.vars["c_port"],
                form.vars["c_con_fail_retry"],
                form.vars["c_password"],
                form.vars["c_src_addr"],
                form.vars["c_bind_npi"],
                form.vars["c_addr_range"],
                form.vars["c_dst_ton"],
                form.vars["c_res_to"],
                form.vars["c_def_msg_id"],
                form.vars["c_priority"],
                form.vars["c_con_loss_retry"],
                form.vars["c_username"],
                form.vars["c_dst_npi"],
                form.vars["c_validity"],
                form.vars["c_requeue_delay"],
                form.vars["c_host"],
                form.vars["c_src_npi"],
                form.vars["c_trx_to"],
                form.vars["c_logfile"],
                form.vars["c_ssl"],
                form.vars["c_loglevel"],
                form.vars["c_bind"],
                form.vars["c_proto_id"],
                form.vars["c_dlr_msgid"],
                form.vars["c_con_loss_delay"],
                form.vars["c_bind_ton"],
                form.vars["c_pdu_red_to"],
                form.vars["c_src_ton"],
            ]
        )
        if not ret:
    ...
    return dict(
        content=form, back=back, title=title, caller="../manage_smpp_connectors"
    )
...
def list_smpp_connectors():
    ...
    for row in tt:
        connector = {}
        connector.update(
            cid=row[0][1:], status=row[1], session=row[2], starts=row[3], stops=row[4]
        )
        connectors.append(connector)
    return connectors
...
def manage_smpp_connectors():
    ...
    db.connector.c_submit_throughput.readable = (
        db.connector.c_submit_throughput.writable
    ) = True

    form = Form(db.connector, dbio=False, formstyle=FormStyleBulma)
    if form.accepted:
        name = form.vars["name"]
        res = jasmin.connector(
            [
                "create",
                name,
                form.vars["c_username"],
...
#     for row in tt:
#         connector = {}
#         connector = dict(uid=row[0][1:], gid=row[1], username=row[2])
#         connectors.append(connector)
#     return connectors
...
def manage_http_connectors():
    ...
    if form.accepted:
        ...
        resp = jasmin.http_cons(
            [
                "create",
                form.vars["hcon_cid"],
                form.vars["hcon_method"],
                form.vars["hcon_url"],
            ]
        )
        if resp:  # we have an error
            ...
            id = db.http_cons.update_or_insert(
                db.http_cons.hcon_cid == form.vars["hcon_cid"],
