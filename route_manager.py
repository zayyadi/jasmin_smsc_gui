...
from pydal.validators import IS_IN_DB, IS_NOT_EMPTY, IS_IN_SET, IS_EMPTY_OR
    ...
    form = Form(
        [
            ...
            Field(
                ...
                requires=IS_IN_DB(
                    db, db.mt_filter._id, db.mt_filter.fid, multiple=True
                ),
                label="Filter(s)",
...
def mt_default(route_type=None):
    ...
    if form.accepted:
        ...
        print("resp", resp)
        if not resp:
            id = db.mtroute.update_or_insert(
                db.mtroute.mt_order == 0,
                mt_order=0,
                mt_type=t,
    ...
    if not route_type:
        ...
        form = Form(
            [
                ...
                Field(
                    ...
                    requires=IS_IN_DB(
                        db, "connector.id", "connector.name", multiple=True
                    ),
        ...
        if form.accepted:
            if len(form.vars["mt_connectors"]) > 1:
                ...
                form.vars["mt_type"] = t
                have_one = (
                    db(db.mtroute.mt_order == form.vars["mt_order"]).select().first()
                )
                if not have_one:
                    resp = jasmin.mtrouter(
                        [
                    ...
                    if not resp:
                        db.mtroute.insert(**db.mtroute._filter_fields(form.vars))
                        flash.set(
                            "Added new Round Robin MT route with order %s" % order
                        )
                        redirect(URL("manage_mt_routes"))
                    else:
                ...
                flash.set(
                    "You need to select at least 2 connectors for round robin route"
                )
        elif form.errors:
...
def mt_failover(route_type):
    ...
    if form.accepted:
        if len(form.vars["mt_connectors"]) > 1:
            ...
            if not have_one:
                resp = jasmin.mtrouter(
                    ["FailoverMTRoute", order, cons[:-1], filters[:-1], rate]
                )
                if not resp:
                    db.mtroute.insert(**db.mtroute._filter_fields(form.vars))
            ...
            flash.set(
                "You need to select more than one connector for this type of route"
            )
    elif form.errors:
...
def mt_routes():
    ...
    for t in tt:
        ...
        if len(t) == 4 or len(t) == 5:  # default route
            ...
            len(t) >= 7 and t[1][0] == "R"
        ):  # Random route can have many connectors and filters
            if t[2] == "0":
                i = 4
            else:
                i = 3
            ...
            len(t) >= 7 and t[1][0] == "F"
        ):  # Failover route can have many connectors and filters
            if t[2] == "0":
                i = 4
            else:
                i = 3
    ...
    if route_type == "DefaultRoute":
            ...
            [
                Field(
                    ...
                    requires=IS_EMPTY_OR(
                        IS_IN_DB(db, "connector.id", "connector.name")
                    ),
                    comment="SMPP connector needs to be available",
                ),
                Field(
                    "mo_user_connectors",
                    "reference user connector",
                    label="User connector",
                    requires=IS_EMPTY_OR(IS_IN_DB(db, "j_user.id", "juser.username")),
                    comment="SMPP connector needs to be available",
                ),
                Field(
                    "mo_http_cons",
                    ...
                    requires=IS_EMPTY_OR(
                        IS_IN_DB(db, "http_cons.id", "http_cons.hcon_cid")
                    ),
                    comment="SMPP connector needs to be available",
                ),
            ],
        ...
        if form.accepted:
            if route_exists(0):
                ...
                redirect(URL("manage_mo_routes"))
            if (
                not form.vars["mo_connectors"]
                and not form.vars["mo_user_connectors"]
                and not form.vars["mo_http_cons"]
            ):
                flash.set("please select either HTTP or SMPP connector")
                redirect(URL("mo_create", "DefaultRoute"))
            if (
                form.vars["mo_connectors"]
                and form.vars["mo_user_connectors"]
                and form.vars["mo_http_cons"]
            ):
            ...
            if form.vars["mo_connectors"]:
                ...
                cons = "smpps(" + con + ")"
            if form.vars["mo_user_connectors"]:
                con = db.j_user[form.vars["mo_user_connectors"]].username
                cons = "smpps(" + con + ")"
            else:
            ...
            if not resp:
                ...
                if id:
                    flash.set(
                        "New Default MO route added with order %s"
                        % form.vars["mo_order"]
                    )
                    redirect(URL("manage_mo_routes"))
                else:
                    flash.set(
                        "New Default MO route with order %s NOT ADDED TO DB"
                        % form.vars["mo_order"]
                    )
            else:
        ...
        form = Form(
            [
                ...
                Field(
                    ...
                    "reference user connector",
                    label="User connector",
                    requires=IS_EMPTY_OR(IS_IN_DB(db, "j_user.id", "j_user.username")),
                    comment="SMPP connector needs to be available",
                ),
                Field(
                    "mo_http_cons",
        ...
        if form.accepted:
            if route_exists(form.vars["mo_order"]):
                flash.set(
                    "MO Route with order %s already exists" % form.vars["mo_order"]
                )
                redirect(URL("manage_mo_routes"))
            if (
                not form.vars["mo_connectors"]
            ...
            if form.vars["mo_connectors"]:
                con = db.connector[form.vars["mo_connectors"]].name
                cons = "smpps(" + con + ");"
            elif form.vars["mo_user_connectors"]:
                con = db.j_user[form.vars["mo_user_connectors"]].username
                cons = "smpps(" + con + ")"
            else:
                con = db.http_cons[form.vars["mo_http_cons"]].hcon_cid
            ...
            if not resp:
                ...
                if id:
                    flash.set(
                        "New Static MO route added with order %s"
                        % form.vars["mo_order"]
                    )
                    redirect(URL("manage_mo_routes"))
                else:
                    flash.set(
                        "New Static MO route with order %s NOT ADDED TO DB"
                        % form.vars["mo_order"]
                    )
            else:
                flash.set("ERROR: %s " % resp)
        ...
        form = Form(
            [
                ...
                Field(
                    ...
                    label="SMPP connector",
                    requires=IS_EMPTY_OR(
                        IS_IN_DB(db, "connector.id", "connector.name", multiple=True)
                    ),
                    comment="SMPP connector needs to be available",
                ),
                Field(
                ...
                Field(
                    ...
                    requires=IS_EMPTY_OR(
                        IS_IN_DB(
                            db, "http_cons.id", "http_cons.hcon_cid", multiple=True
                        )
                    ),
                    comment="HTTP connector needs to be available",
                ),
                Field(
                    ...
                    "list:reference mt_filter",
                    requires=IS_IN_DB(
                        db, "mt_filter.id", "mt_filter.fid", multiple=True
                    ),
                    label="Filter(s)",
                    comment="Filters need to be added prior to adding routes. Please see filter management",
                ),
        ...
        if form.accepted:
            ...
            if (
                ...
                flash.set("please select HTTP, User or SMPP connector")
                redirect(URL("mo_create", "StaticMORoute"))
            if (
                form.vars["mo_connectors"]
                and form.vars["mo_user_connectors"]
            ...
            form.vars["mo_type"] = route_type
            order = form.vars["mo_order"]
            resp = jasmin.morouter(
                ["RandomRoundrobinMORoute", order, cons[:-1], filters[:-1]]
            )
            if not resp:
                id = db.moroute.insert(**db.moroute._filter_fields(form.vars))
                if id:
                    flash.set(
                        "New Random Roundrobin MO route added with order %s"
                        % form.vars["mo_order"]
                    )
                    redirect(URL("manage_mo_routes"))
                else:
                    flash.set(
                        "New Random Roundrobin MO route with order %s NOT ADDED TO DB"
                        % form.vars["mo_order"]
                    )
            else:
                flash.set("ERROR: %s " % resp)
        ...
        if form.accepted:
            ...
            if not resp:
                ...
                if id:
                    flash.set(
                        "New Failorver MO route added with order %s"
                        % form.vars["mo_order"]
                    )
                    redirect(URL("manage_mo_routes"))
                else:
                    flash.set(
                        "New Failorver MO route with order %s NOT ADDED TO DB"
                        % form.vars["mo_order"]
                    )
            else:
                flash.set("ERROR: %s " % resp)
    ...
    if not rows:
        ...
        for t in tt:
            ...
            if len(t) == 3:  # default route
                route.update(
                    r_order=t[0][1:], r_type=t[1], r_connectors=t[2], r_filters=""
                )
            elif (
                len(t) > 3 and t[1][0] == "R"
            ):  # Random route can have many connectors and filters
